...
function registration() {
    axios.get(`https://dexone.ru/backend_calorie/users`).then((res) => {
        ...
        if (logins.includes(regLogin.value) == false) { //если логин не найден в бд допускается регистрация
            ...
            axios.post(`https://dexone.ru/backend_calorie/data`, { id: lastId + 1, name: regLogin.value[0].toUpperCase() + regLogin.value.slice(1), max: "0", info:  ["0",[],[],[]]}) //создание data

            userStore.axiosInfo = { id: lastId + 1, name: regLogin.value[0].toUpperCase() + regLogin.value.slice(1), max: 0, info:  ["0",[],[],[]]} //обновляет стор
        }
        else {
